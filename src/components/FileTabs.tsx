...
const getFileIcon = (name: string) => {
  ...
  if (name.endsWith('.css')) return <FileText className="w-3.5 h-3.5 text-[hsl(210,80%,60%)]" />;
  if (name.endsWith('.js') || name.endsWith('.ts')) return <FileJson className="w-3.5 h-3.5 text-[hsl(50,85%,55%)]" />;
...
const FileTabs = ({ files, activeFileId, onSelectFile, onAddFile, onDeleteFile }: FileTabsProps) => {
  ...
  return (
    <div className="flex items-center bg-[hsl(var(--editor-bg))] border-b border-border overflow-x-auto">
      {files.map((file) => (
        <button
          key={file.id}
        ...
        >
          ...
          {files.length > 1 && (
            <X
              className="w-3 h-3 opacity-0 group-hover:opacity-100 hover:text-destructive transition-opacity ml-1"
              onClick={(e) => {
                e.stopPropagation();
